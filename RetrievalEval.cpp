		     ...
		     Index *ind)
{
  ...
  return docTermFreq*qryTermWeight*log(double(ind->docCount()/ind->docCount(termID)));
}
		     ...
		     Index *ind)
{
  ...
  return (log(docTermFreq)+1)*qryTermWeight*log(double(ind->docCount()/ind->docCount(termID)));
}
		     ...
		     Index *ind)
{
  ...
  return double(docTermFreq/(docTermFreq+0.5+(1.5*double(ind->docLength(docID)/ind->docLengthAvg()))))*(double(8+qryTermWeight)/double(7+qryTermWeight))*(log(double((ind->docCount() - ind->docCount(termID) + 0.5)/(ind->docCount(termID) + 0.5))));
}
